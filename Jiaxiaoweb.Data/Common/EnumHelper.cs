...
namespace Jiaxiaoweb.Common
{
    public static class EnumHelper
    {
        ...
        public static string GetEnumDescriptionFromDataTableByValue(object e,Type type) {
            string desc = string.Empty;
            DataTable dt = EnumListTable(type);
            foreach (DataRow dr in dt.Rows) { 
                if(dr["Value"].ToString() == e.ToString()){
            ...
            foreach (FieldInfo field in fields)
            {
                ...
                if (field.FieldType.IsEnum == true)
                {
                    ...
                    if (arr.Length > 0)
                    {
                        ...
                        dr["Description"] = aa.Description;
                    }
                    else
                    {
                        //如果没有特性描述（-_-!忘记写了吧~）那么就显示英文的字段名
                        dr["Description"] = field.Name;
                    }
                    dt.Rows.Add(dr);
                }
            }
